...
def update_notice():
    ...
    """
    try:
        installed_version = get_version("vnstock3")
        ...
        if latest_version and version.parse(installed_version) < version.parse(latest_version):
            message = (
                f"Thư viện `vnstock3` đã được hợp nhất với tên gọi `vnstock`. Phiên bản mới nhất {latest_version} đã có mặt."
                "Vui lòng chuyển đổi gói thư viện thành `vnstock` để luôn nhận bản cập nhật với câu lệnh : `pip install vnstock --upgrade`.\n"
